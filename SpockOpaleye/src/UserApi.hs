...
create_user conn user@User
  ...
  ids <- runInsertManyReturning
           ...
           userTable
           (return
              (Nothing, constant tenant_id, pgStrictText username, constant password, toNullable . pgStrictText <$> first_name, toNullable . pgStrictText <$> last_name, constant
                                                                                                                                                                           status))
           (\(id_, _, _, _, _, _, _) -> id_)
  return $
    case ids of
...
update_user conn (UserId tid) (User
  ...
  runUpdate
    ...
    userTable
    (\(id, _, _, _, _, _, _) ->
       (Just id, constant tenant_id, pgStrictText username, constant password, toNullable . pgStrictText <$> firstname, toNullable . pgStrictText <$> lastname, constant
                                                                                                                                                                  status))
...
remove_role_from_user conn t_user_id t_role_id =
  runDelete conn userRolePivotTable
    (\(user_id, role_id) -> (user_id .== constant t_user_id) .&& (role_id .== constant t_role_id))

make_user :: (UserId, TenantId, Text, BcryptPassword, Maybe Text, Maybe Text, UserStatus)
          -> User
