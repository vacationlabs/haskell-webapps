...
instance QueryRunnerColumnDefault ProductId ProductId where
  queryRunnerColumnDefault = fieldQueryRunnerColumn
-- For TenantId
instance FromField TenantId where
  fromField field mb_bytestring = TenantId <$> fromField field mb_bytestring

instance QueryRunnerColumnDefault TenantId TenantId where
  queryRunnerColumnDefault = fieldQueryRunnerColumn
...
instance Default Constant TenantId (Maybe (Column TenantId)) where
  def = Constant (\(TenantId x) -> Just $ unsafeCoerceColumn $ pgInt4 x)

instance Default Constant () (Maybe (Column TenantId)) where
